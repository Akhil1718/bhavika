...
logoutBtn.onclick = async () => {
  ...
  await refreshMe();
  setAuthUi();
  alert('Logged out. Wishlist/cart stay saved in your account.');
};

window.openFromCart = async (productId) => {
  closeOverlay();
...
function renderCartOverlay() {
  const lines = state.me.cart.map((c) => {
    ...
    if (!p) return '';
    return `<div class='cart-item' onclick='openFromCart("${p.id}")'>
      <img src='${p.images[0]}' alt='${p.name}' />
      <div>
        <h4>${p.name}</h4>
        <p>${p.brand}</p>
        <p>${c.quantity} × $${p.salePrice} = $${(p.salePrice * c.quantity).toFixed(2)}</p>
      </div>
    </div>`;
  }).join('') || '<p>Cart is empty.</p>';

  overlay.classList.remove('hidden');
  overlay.innerHTML = `<div class='modal'><h3>YOUR CART</h3><div class='cart-list'>${lines}</div><button onclick='checkoutAll()'>Checkout All</button><button class='ghost' onclick='closeOverlay()'>Close</button></div>`;
}
